...
let addnode currentnodes = function
  | (x, _, y) ->
    if not (find x currentnodes) then x :: currentnodes
    else if not (find y currentnodes) then y :: currentnodes
    else currentnodes;;
...
let getedges (graph : graph) x =
  let rec aux (graph : graph) vertice (lst : graph) =
    ...
    | [] -> lst
    | (x, w, y) :: tail ->
        if x = vertice || y = vertice then aux tail vertice ((x, w, y) :: lst)
        else aux tail vertice lst
  in aux graph x [];;
...
let removeedges currentnodes graph =
  let rec aux (graph : graph) (edges : graph) (currentnodes : int list) =
    ...
    | [] -> graph
    | (x, w, y) :: xs ->
        if find x currentnodes && find y currentnodes then aux (remove graph (x, w, y)) xs currentnodes
        else aux graph xs currentnodes
...
let mst (graph : graph) =
  let rec aux (graph : graph) (tree : graph) (currentnodes : int list) =
    ...
    | _ ->
        aux
          (remove (removeedges currentnodes graph) (skinny (union graph currentnodes)))
          (skinny (union graph currentnodes) :: tree)
